...
#输出指定某一天背的单词
def everyday_words(volcaulay_list,time,day):
    totle_number = len(volcaulay_list)
    everyday_number = totle_number //(time-1)
    words_day = []
    start= (day-1)*everyday_number+1
    if day == time:
        end = totle_number
    else:
        end = day*everyday_number
    for num in range(start,end):
        val = volcaulay_list[num]
        words_day.append([val[0],val[3]])
    return words_day
#输出所有日子的单词
def everyday_words_list(meanings_words,time):
    for index in range(1,time + 1):
        everyday_word = everyday_words(meanings_words, time, index)
        file_path = 'output_everyday_word/day' + str(index) + '.csv'
        print_to_csv(everyday_word,file_path)
...
def main():
    ...
    # 输出文件
    print_to_csv(meanings_words, 'output/test2copy.csv')
    number_totle_words = len(meanings_words)
    print number_totle_words
    time = 30
    day = 20
    everyday_word = everyday_words(meanings_words, time, day)
    print_to_csv(everyday_word, 'output/homeworkday6.csv')
    everyday_words_list(meanings_words, time)
